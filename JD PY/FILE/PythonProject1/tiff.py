class tiff():
    ...
    def __init__(self):       # call resources to process
        self.resources = {"flour_A":100,"flour_B":200,"flour_C":300,"water":400}  #after resources and menu
    ...
    def report(self):
        for item,price in self.resources.items():
    ...
    def payment(self,price):
        ...
        if user_money < price:
            print(f"{user_money} is less than {price}")
            return False
        change = user_money-price
        if change > 0:
            print(f"here is the {change}")
            self.resources["price"] += price
            return True
